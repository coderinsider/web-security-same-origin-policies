(function () {
  ...
  $('#roses').on('click', function(e) {
    e.preventDefault();
    window.open('https://[[RESOURCES]]/pages/product-detail-arrangements.html');
  });
  window.addEventListener('DOMContentLoaded', function() {
    if (window.opener) {
      window.opener.postMessage('ready', 'https://[[SITE]]');
    }
  });
  window.addEventListener('message', function(e) {
    if ((e.origin !== 'https://[[RESOURCES]]') &&
        (e.origin !== 'https://[[SITE]]')) {
           return;
         }
    console.log(e.data);
    if (e.data === 'ready') {
      e.source.postMessage('Two-way communication established', 'https://[[RESOURCES]]');
    }
  });
})();